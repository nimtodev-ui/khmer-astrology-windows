...
| `ប្រតិទិនស្វ័យប្រវត្តិ` | `A5:Q371`, `R6:X371` | Date rows derive weekday, lunar day/phase, tithi name, lunar month, animal year, Sesa-Kala-Yoga, Yuga, and related era fields from the day-by-day lunar walk (R = Khmer year, S = year type, T = lunar month, U = lunar day, V = month length) | User-specific traditional calendar date | `AutomaticCalendarCalculator` / `KhmerCalendarCalculator` | `GetLunarState` / `ApplyDateLevelCalendar` (`KhmerLunarDateTests`) |

The birth-date lunar fields previously used one month code per Gregorian month plus a 30-day lunar-month assumption, which gave the wrong lunar month after a lunar month began inside a Gregorian month and drifted on 29-day months (123 of 366 days differed in 2024; e.g. 17 Oct 2024, the end of Buddhist Lent, was reported as Bhadrapada instead of Assayuja). They now read the R:V walk, matching sheet 31 and the festival dates in `KhmerLunarDateTests`.
